...
namespace OSTL.UPnP
{
    ...
    public sealed class UPnPDeviceComparer_Type : IComparer
    {
        ...
        {
            // 2TODO:  Add UPnPDeviceComparer_Type.Compare implementation
    ...
    public sealed class UPnPDevice
    {
        ...
        public Image Icon
        {
            ...
            set
            {
                if (ControlPointOnly == false) _icon = value;
            }
        }
        public Image Icon2
        {
            ...
            set
            {
                if (ControlPointOnly == false) _icon2 = value;
            }
        }
        ...
        public Icon favicon
        {
            ...
            set
            {
                if (ControlPointOnly == false) _favicon = value;
            }
        }
        ...
        public UPnPDevice[] GetDevices(string urn)
        {
            ...
            if (DeviceURN.ToLower().StartsWith(urn.ToLower()))
            {
                t.Add(this);
            }
            foreach (UPnPDevice d in EmbeddedDevices)
            {
                if (d.DeviceURN.ToLower().StartsWith(urn.ToLower()))
                {
                    t.Add(d);
                }
            }
        ...
        public UPnPService[] GetServices(string urn)
        {
            ...
            {
                if (s.ServiceURN.ToLower().StartsWith(urn.ToLower()))
                {
                    t.Add(s);
                }
            }
        ...
        {
            if (UniqueDeviceName == UDN) return (this);

            UPnPDevice RetVal = null;
            foreach (UPnPDevice ed in EmbeddedDevices)
            {
        ...
        public override int GetHashCode()
        {
            if (ControlPointOnly)
            {
                ...
                {
                    return (descXmlLocation.GetHashCode());
                }
        ...
        {
            device.ExpirationTimeout = ExpirationTimeout;
            device.parent = this;
            device.IsRoot = false;
            ...
            if (ControlPointOnly == false)
            {
                ...
                AddVirtualDirectory(device.UniqueDeviceName, device.HandleParent_Header, device.HandleParent);
            }
            else
            {
                AddVirtualDirectory(device.UniqueDeviceName, null, device.EventProcesser);
        ...
        {
            d.InterfaceToHost = InterfaceToHost;
        ...
        private void UpdateDeviceSink(UPnPDeviceFactory sender, UPnPDevice d, Uri LocationUri)
        {
            ...
            BaseURL = d.BaseURL;
            InterfaceToHost = d.InterfaceToHost;
        ...
        private void UpdateDeviceSink2(UPnPDevice d)
        {
            ...
            {
                UPnPService MatchingService = GetService(s.ServiceID);
        ...
        private void ManualNewDeviceInterface(IPAddress ip, int Port)
        {
            ...
            if ((OnSniff != null) || (OnSniffPacket != null)) WebServer.OnSession += SniffSessionSink;
        ...
        public void StartDevice(IPEndPoint Manual)
        {
            if (ControlPointOnly == false)
            {
                if (IsRoot)
                {
                    ManualIPEndPoint = Manual;
                    ManualNewDeviceInterface(Manual.Address, Manual.Port);
                    Advertise();
                }
                else
                {
        ...
        public void StopDevice()
        {
            if (ControlPointOnly == false)
            {
                if (IsRoot)
                {
                    ...
                    {
                        if (ManualIPEndPoint != null) SendBye(ManualIPEndPoint.Address);
                    }
        ...
        private HTTPMessage Get(String GetWhat, IPEndPoint local)
        {
            ...
            if ((GetWhat == "icon.png") && (_icon != null))
            {
                lock (_icon)
                {
                    ...
                    _icon.Save(mstm, ImageFormat.Png);
                    msg.StatusCode = 200;
                    msg.StatusData = "OK";
        ...
        private void SendNotify()
        {
            ...
            if (NetInfo == null)
            {
                if (ManualIPEndPoint != null)
                {
                    SendNotify(ManualIPEndPoint.Address);
                }
            }
            else
            {
            ...
            for (id = 0; id < Services.Length; ++id)
            {
                if (Services[id].ControlURL == Control)
                {
                    if (MethodTag != "QueryStateVariable")
                    {
                        ...
                        {
                            if (arg.IsReturnValue) iis.RetArg = (UPnPArgument)arg.Clone();
            ...
            if (ControlPointOnly)
            {
                ...
                if (InterfaceToHost != null)
                {
                    try
                    {
                        ...
                        td.AddVirtualDirectory(UniqueDeviceName, null, EventProcesser);

                        if (InterfaceToHost.AddressFamily == AddressFamily.InterNetwork)
                        {
                            service.EventCallbackURL = "http://" + InterfaceToHost.ToString() + ":" + mws.LocalIPEndPoint.Port.ToString() + "/" + UniqueDeviceName + "/" + service.ServiceID;
                        }
                        else if (InterfaceToHost.AddressFamily == AddressFamily.InterNetworkV6)
                        {
        ...
        private void SetVersion(string v)
        {
            ...
            Major = 0;
            int.TryParse(mj, out Major);
            Minor = 0;
            int.TryParse(mn, out Minor);
        ...
        static internal UPnPDevice Parse(String XML, Uri source, IPAddress Intfce)
        {
                    ...
                    while ((XMLDoc.LocalName != "root") && (XMLDoc.EOF == false))
                    {
                        ...
                        switch (XMLDoc.LocalName)
                        {
                            case "specVersion":
                                ...
                                //2ToDo: This is supposed to be Arch_Major and Arch_Minor
            ...
            {
                EventLogger.Log(ex, "Invalid UPnP Device Description\r\nLine " + (XMLDoc.LineNumber).ToString() +
        ...
        internal void FetchIcon(Uri IconUri)
        {
            ...
            httprequestor.OnRequestCompleted += HandleIcon;
        ...
        void HandleIcon(HttpRequestor sender, bool success, object tag, string url, byte[] data)
        {
            ...
            if (success)
            {
                Image i = Image.FromStream(new MemoryStream(data));
                if (i != null) _icon = i;
            }
        }
        ...
        public byte[] GetRootDeviceXML(IPEndPoint local)
        {
            ...
            XmlTextWriter XDoc = new XmlTextWriter(mstream, Encoding.UTF8);
            ...
            if (_BootID != "")
            {
                XDoc.WriteAttributeString("configId", _BootID);
            }
        ...
        private void GetNonRootDeviceXML(IPEndPoint local, XmlTextWriter XDoc)
        {
            ...
            if (_icon != null)
            {
                lock (_icon)
                {
                    ...
                    XDoc.WriteElementString("depth", Image.GetPixelFormatSize(_icon.PixelFormat).ToString());
                    ...
                    if (_icon2 != null)
                    {
                        ...
                        XDoc.WriteElementString("width", _icon2.Width.ToString());
                        XDoc.WriteElementString("height", _icon2.Height.ToString());
                        XDoc.WriteElementString("depth", Image.GetPixelFormatSize(_icon2.PixelFormat).ToString());
