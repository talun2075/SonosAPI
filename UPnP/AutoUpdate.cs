...
namespace OSTL.UPnP
{
    public partial class AutoUpdate : Form
    {
        ...
        private static string TempFolder { get
        {
            if (ShortProductName == null) return "OpenToolsTemp";
            return ShortProductName.Replace(" ", "");
        }
        }
        ...
        public static void ShowMainSite()
        {
            ...
            {
                try { Process.Start("http://opentools.homeip.net"); }
                catch (Win32Exception ex) 
                {
                    EventLogger.Log(ex);
                }
        ...
        {
            if (File.Exists(Application.StartupPath + "\\UpdateInfo.ini") == false) return false;

            // Fetch the service settings
            string[] settings = File.ReadAllLines(Application.StartupPath + "\\UpdateInfo.ini", Encoding.UTF8);
        ...
        private static void requestor_OnRequestCompleted(HttpRequestor sender, bool success, object tag, string url, byte[] data)
        {
            if (success)
            {
                ...
                if (existingversion < newversion)
                {
                    ...
                    parentform.Invoke(new ThreadStart(ShowUpdateForm));
                }
            }
            requestor.OnRequestCompleted += requestor_OnRequestCompleted;
            requestor = null;
