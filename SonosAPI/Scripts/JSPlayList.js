    ...
    this.RenderPlaylist = function (stream) {
        ...
        if (stream === true || this.CheckIsEmpty() === true) {
            ...
            for (var i = 0; i < this.Playlist.length; i++) {
                ...
                $('<div id="Currentplaylist_' + (i) + '" class="currentplaylist"><DIV class="currentrackinplaylist" onclick="ShowSongInfos(this)">' + item.Title + '</div><DIV class="curpopdown"><DIV class="playlistcover" data-geladen="NichtGeladen" data-url="' + songcover + '" data-uri="' + item.Uri + '" data-plid="'+item.ID+'"></DIV><DIV class="playlistplaysmall" onclick="PlayPressSmall(this)"></DIV><DIV class="mediabuttonsmal" onclick="RemoveFromPlaylist(this);return false;"><img src="Images/erase_red.png"></DIV><div class="bomb" onclick="ShowPlaylistRating(this)"><img src="/Images/bombe.png" alt="playlistbomb"/></DIV><DIV onclick="ShowPlaylistRating(this)" class="rating_bar" style="margin-top: 14px;" Style="float:left;"><DIV style="width:0%;"></DIV></DIV><div OnMouseOver="MakeCurrentPlaylistSortable()" OnTouchStart="MakeCurrentPlaylistSortable()" OnTouchEnd="ResortPlaylistDisable()" OnMouseOut="ResortPlaylistDisable()" class="moveCurrentPlaylistTrack"></div><DIV class ="addFavItemPlaylist" onclick="AddFavItem(this,\'playlist\');"></DIV></DIV></div>').appendTo(SoDo.currentplaylistwrapper);
            }
            SoDo.playListLoader.slideUp();
